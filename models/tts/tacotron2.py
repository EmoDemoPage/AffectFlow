...
from models.commons.rnn import ConvNorm, LinearNorm
from utils.nn.seq_utils import get_mask_from_lengths
...
class Tacotron2(nn.Module):
    def __init__(self, dict_size, hparams, out_dims=80):
        ...
        self.embedding = nn.Embedding(dict_size, hparams["symbols_embedding_dim"])
        std = sqrt(2.0 / (dict_size + hparams["symbols_embedding_dim"]))
        ...
        self.postnet = Postnet(
            out_dims, postnet_embedding_dim, postnet_kernel_size, postnet_n_convolutions
        )

    def forward(self, txt_tokens, txt_lengths, mels, mel_lengths):
        ret = {}
        embedded_inputs = self.embedding(txt_tokens).transpose(1, 2)

        encoder_outputs = self.encoder(embedded_inputs, txt_lengths)

        mel_outputs, gate_outputs, alignments = self.decoder(
            encoder_outputs, mels.transpose(1, 2), memory_lengths=txt_lengths
        )
        ...
        ret["decoder_output"] = mel_outputs.transpose(1, 2)
        ret["mel_out"] = mel_outputs_postnet.transpose(1, 2)
        ret["stop_preds"] = gate_outputs
        ret["alignments"] = alignments.transpose(1, 2)
        return ret
