...
class CBAD(nn.Module):
    def __init__(
        self, in_dim, out_dim, kernel_size, stride, padding, bias, activation, dropout
    ):
...
class TransformerTTS(nn.Module):
    def __init__(self, dict_size, hp, out_size=None):
        super(TransformerTTS, self).__init__()
        self.hp = hp
        self.Embedding = nn.Embedding(dict_size, hp.symbols_embedding_dim)
        ...
        self.Encoder = nn.ModuleList(
            [
                TransformerEncoderLayer(
                    d_model=hp.hidden_dim, nhead=hp.n_heads, dim_feedforward=hp.ff_dim
                )
    ...
    def forward(self, text, text_lengths, melspec, mel_lengths):
        ret = {}
        ### Size ###
        text = text[:, : text_lengths.max().item()]
        melspec = melspec[:, :, : mel_lengths.max().item()]
        outputs = self.outputs(text, melspec.transpose(1, 2), text_lengths, mel_lengths)

        mel_out, mel_out_post = outputs[0], outputs[1]
        enc_dec_alignments = outputs[4]
        gate_out = outputs[5]
        ret["mel_out"] = mel_out.transpose(1, 2)
        ret["mel_out_post"] = mel_out_post.transpose(1, 2)
        ret["enc_dec_alignments"] = enc_dec_alignments
        ret["gate_out"] = gate_out
    ...
    def inference(self, text, max_len=1024):
        ...
        ### Transformer Decoder ###
        mel_input = text.new_zeros(1, self.hp.n_mel_channels, max_len).to(torch.float32)
        dec_alignments = text.new_zeros(
            self.hp.n_layers, self.hp.n_heads, max_len, max_len
        ).to(torch.float32)
        # enc_dec_alignments = text.new_zeros(
        #     self.hp.n_layers, self.hp.n_heads, max_len, text.size(1)
        # ).to(torch.float32)
        enc_dec_alignments = text.new_zeros(
            self.hp.n_layers, self.hp.n_heads, text.size(1), max_len
        ).to(torch.float32)
        ### Generation ###
        stop = []
        for i in range(max_len):
            tgt = (
                self.Prenet_D(mel_input.transpose(1, 2).contiguous())
                .transpose(0, 1)
                .contiguous()
            )
        ...
        mel_out_post = mel_out_post.contiguous().transpose(1, 2) + mel_out
        ret["mel_out"] = mel_out_post
        ret["enc_alignments"] = enc_alignments
        ret["dec_alignments"] = dec_alignments
        ret["enc_dec_alignments"] = enc_dec_alignments
        ret["stop"] = stop
