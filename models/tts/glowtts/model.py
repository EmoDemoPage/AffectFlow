...
# from models.tts.glowtts.modules import ConvReluNorm, ActNorm, InvConvNear
# from models.tts.glowtts.commons import sequence_mask, squeeze, unsqueeze, generate_path
# from models.tts.glowtts.attentions import LayerNorm, Encoder, CouplingBlock
from .modules import ConvReluNorm, ActNorm, InvConvNear
from .commons import sequence_mask, squeeze, unsqueeze, generate_path
from .attentions import LayerNorm, Encoder, CouplingBlock
...
class GlowTTS(nn.Module):
    def __init__(self, dict_size, hparams, out_dims=80):
        ...
        self.encoder = TextEncoder(
            dict_size,
            out_dims,
            hparams["hidden_channels_enc"],
            hparams["filter_channels"],
            hparams["filter_channels_dp"],
            hparams["n_heads"],
            hparams["n_layers_enc"],
            hparams["kernel_size"],
            hparams["p_dropout"],
            window_size=hparams["window_size"],
            mean_only=hparams["mean_only"],
            prenet=hparams["prenet"],
            gin_channels=hparams["gin_channels"],
        )

        self.decoder = FlowSpecDecoder(
            out_dims,
            hparams["hidden_channels_dec"],
            hparams["kernel_size_dec"],
            hparams["dilation_rate"],
            hparams["n_blocks_dec"],
            hparams["n_block_layers"],
        ...
        if hparams["n_spks"] > 1:
            self.emb_g = nn.Embedding(hparams["n_spks"], hparams["gin_channels"])
